...
// Must match process-messages/route.ts
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Same join shape as cron (service role — bypasses RLS)
        ...
        const rules = Array.isArray(message.delivery_rules) ? message.delivery_rules : [message.delivery_rules];
        ...
        // --- Mirror exact cron conditions ---
        const status_is_scheduled = message.status === "scheduled";
        const mode_is_date = rule?.mode === "date";
        const deliver_at_is_due = rule?.deliver_at != null && new Date(rule.deliver_at) <= now;
        const claimedAt = message.delivery_claimed_at ? new Date(message.delivery_claimed_at as string) : null;
        ...
        if (!status_is_scheduled)
            blocking_reasons.push(`status is '${message.status}' — expected 'scheduled'`);
        if (!mode_is_date)
            blocking_reasons.push(`delivery mode is '${rule?.mode ?? "missing"}' — expected 'date'`);
