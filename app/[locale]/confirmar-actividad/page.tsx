...
function ConfirmActivityContent() {
    ...
    useEffect(() => {
        async function confirm() {
            try {
                ...
                if (token) {
                    // Token-based confirmation (from email link — no login required)
                    ...
                    // Session-based confirmation (user is already logged in)
        ...
        <>
            ...
            {status === 'success' && (
                <div className="space-y-4">
                    ...
                    <p className="text-sm text-gray-500">Tus mensajes siguen protegidos.</p>
                    <p className="text-sm text-gray-400">Podés cerrar esta pestaña.</p>
                </div>
            )}
            ...
            {status === 'error' && (
                <div className="space-y-4">
                    ...
                    <p className="text-sm text-gray-500">El enlace puede haber expirado. Podés confirmar tu actividad iniciando sesión en tu panel.</p>
                </div>
            )}
        </>
    )
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-[#F0EBE3] font-serif">
            <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-sm border border-[#E0D8CC] text-center">
                ...
                <Suspense fallback={
                    <div className="space-y-4">
                        <div className="animate-spin w-12 h-12 border-4 border-[#C4623A] border-t-transparent rounded-full mx-auto"></div>
                        <p className="text-[#3a2e24]">Cargando...</p>
                    </div>
                }>
