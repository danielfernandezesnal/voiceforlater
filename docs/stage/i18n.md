...
- **Quality Assurance**: Added dictionary smoke test (commit `07ddf27`) to ensure key parity between `en` and `es`.
...
- Partial translations → `dictionary.smoke.test.ts` identifies missing keys during test phase.
