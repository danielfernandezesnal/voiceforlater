                ...
                {steps.map((label, index) => {
                    ...
                    return (
                        <div key={label} className="flex-1 flex flex-col items-center relative">
                            {/* Connector Line that spans to the center of the next item */}
                            {!isLast && (
                                <div
                                    className={`absolute top-[18px] sm:top-5 left-[calc(50%+18px)] sm:left-[calc(50%+20px)] w-[calc(100%-36px)] sm:w-[calc(100%-40px)] h-[2px] transition-colors duration-500 z-0 ${stepNumber < currentStep ? 'bg-success/40' : 'bg-border/20'}`}
                                />
                            )}
                            ...
                            >
                                ...
                                <span
                                    className={`relative z-10 mt-2 sm:mt-3 text-[9px] sm:text-xs text-center uppercase tracking-tight sm:tracking-widest whitespace-nowrap transition-colors duration-300 ${isActive
