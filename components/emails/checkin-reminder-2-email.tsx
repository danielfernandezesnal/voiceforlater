import * as React from 'react';
import { getDictionary, Locale } from '@/lib/i18n';
import { getResend, DEFAULT_SENDER } from '@/lib/resend';
import { CheckinReminderLayout } from './_shared/checkin-reminder-layout';
import { render } from '@react-email/render';

export interface CheckinReminder2EmailProps {
  checkinUrl: string;
  locale: Locale;
}

export async function CheckinReminder2Email({
  checkinUrl,
  locale,
}: CheckinReminder2EmailProps) {
  const dict = await getDictionary(locale);
  const t = dict.emails.checkinReminder2;
  const common = dict.emails.common;
  const dashboardUrl = checkinUrl.replace(/confirmar-actividad.*/, 'dashboard').replace(/verify-status.*/, 'dashboard');
  const supportUrl = checkinUrl.replace(/confirmar-actividad.*/, 'contact').replace(/verify-status.*/, 'contact');

  return (
    <CheckinReminderLayout
      locale={locale}
      title={t.subject}
      heroTitle={t.subject}
      checkinUrl={checkinUrl}
      dashboardUrl={dashboardUrl}
      supportUrl={supportUrl}
      common={common}
      body={t.body}
    />
  );
}

export async function sendCheckinReminder2Email(toUserEmail: string, checkinUrl: string, locale: Locale) {
  const resend = getResend();
  const dict = await getDictionary(locale);

  if (!resend) {
    console.error("Resend client not available");
    return { error: "Resend client not available" };
  }

  try {
    const { data, error } = await resend.emails.send({
      from: DEFAULT_SENDER,
      to: toUserEmail,
      subject: dict.emails.checkinReminder2.subject,
      html: await render(await CheckinReminder2Email({ checkinUrl, locale })),
    });
    if (error) {
      console.error("Error sending Checkin Reminder 2:", error);
      return { error };
    }
    return { data };
  } catch (error) {
    console.error("Error sending Checkin Reminder 2:", error);
    return { error };
  }
}
